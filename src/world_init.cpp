...
Entity createBossRobot(RenderSystem* renderer, vec2 position)
{
	...
	r.search_box = { 20 * 64.f,20 * 64.f };
	r.attack_box = { 12* 64.f,12 * 64.f };
	r.panic_box = { 4 * 64.f,4 * 64.f };

	auto& bossRobotAnimation = registry.bossRobotAnimations.emplace(entity);
	bossRobotAnimation = BossRobotAnimation(64, 640, 1280);
	bossRobotAnimation.setState(BossRobotState::IDLE, Direction::LEFT);
	registry.renderRequests.insert(
		entity,
		{
			TEXTURE_ASSET_ID::BOSSROBOT,
			EFFECT_ASSET_ID::TEXTURED,
			GEOMETRY_BUFFER_ID::SPRITE
		});
...
Entity createProjectile(vec2 position,vec2 speed,float angle,bool ice, bool player_projectile, bool boss_projectile) {
	auto entity = Entity();
