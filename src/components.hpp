...
public:
	BossRobotAnimation(int sprite_size = 64, int s_width = 640, int s_height = 1280)
		: BaseAnimation(sprite_size, s_width, s_height) {}
	...
	int getMaxFrames() const override {
		switch (current_state) {
		case BossRobotState::WALK: return 7;
		case BossRobotState::IDLE: return 4;
		case BossRobotState::DEAD: return 8;
		case BossRobotState::HURT: return 3;
		case BossRobotState::ATTACK: return 10;
	...
	bool loop() const override {
		return current_state == BossRobotState::WALK;
	}
...
{
	// need to have health - after attacking - how much damage does it do.
	float current_health = 300;  // Current health value
	float max_health = 300;      // Max health value
	bool should_die = false;
	float death_cd;
