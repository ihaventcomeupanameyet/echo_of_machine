...
//rintf("elapsed_ms_since_last_update: %f, current_speed: %f\n", elapsed_ms_since_last_update, current_speed);
	...
	if (!key_spawned && registry.bossRobotAnimations.components.size() == 0 && total_boss_robots_spawned == TOTAL_BOSS_ROBOTS) {
		//&& next_key_spawn < 0.f 
		printf("Spawning key!\n");
...
void WorldSystem::load_boss_level(int map_width, int map_height) {
	const std::vector<std::pair<float, float>> BOSS_ROBOT_SPAWN_POSITIONS = {
	{64.f * 36, 64.f * 56}
	};

	for (size_t i = total_boss_robots_spawned; i < BOSS_ROBOT_SPAWN_POSITIONS.size(); ++i) {
		...
		const auto& pos = BOSS_ROBOT_SPAWN_POSITIONS[i];
		createBossRobot(renderer, vec2(pos.first, pos.second));

		// Update the count of total robots spawned
		total_boss_robots_spawned++;

		// Check if we�ve reached the total spawn limit for robots
		if (total_boss_robots_spawned >= TOTAL_BOSS_ROBOTS) {
			break;
		}
	}
	...
	if (action == GLFW_PRESS) {
		...
		if (key == GLFW_KEY_T) {
			if (player_data.current_stamina >= 15.0f &&
				...
				Entity proj = createProjectile(motion.position, proj_speed, angle, false, true, false);
